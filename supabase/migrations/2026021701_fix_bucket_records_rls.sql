...
-- 20260211_complete_rls.sql target 'bucket_records'.
...
-- Drop the stale bucket_records policies if that table doesn't exist
-- (If bucket_records does exist as a legacy table, leave its policies alone)
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM information_schema.tables
    WHERE table_schema = 'public'
        AND table_name = 'bucket_records'
) THEN RAISE NOTICE 'bucket_records table does not exist — no stale policies to clean up';
END IF;
END $$;