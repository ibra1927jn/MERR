...
test.describe('Operational Chaos — Daily Human Errors', () => {
  ...
  // ------------------------------------------
  test('Ghost Worker: checked-in picker with 0 buckets triggers compliance alert', async ({
    page,
  }) => {
    ...
    const warningIcon = page
      .locator('[data-testid="compliance-alert"]')
  ...
  // ------------------------------------------
  test('Forgotten Checkout: TimesheetEditor shows warning for missing checkout', async ({
    page,
  }) => {
    const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
    const orchardId = 'a1000000-0000-0000-0000-000000000001';
  ...
  test('Ghost Payroll: export handles 0-bucket picker without NaN', async ({ page }) => {
    ...
    if (await exportTab.isVisible()) {
      ...
      if (await exportBtn.first().isVisible()) {
        ...
        const nanErrors = consoleErrors.filter(
          e => e.includes('NaN') || e.includes('Infinity') || e.includes('division')
        );
