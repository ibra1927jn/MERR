...
vi.mock('@/repositories/auth-context.repository', () => ({
  authContextRepository: {
    ...
    getAllOrchards: vi
      .fn()
      .mockResolvedValue([{ id: 'orchard-1', name: 'Test Orchard', total_rows: 10 }]),
  },
}));
vi.mock('@/services/db', () => ({
  db: {
    delete: vi.fn().mockResolvedValue(undefined),
    user_cache: { get: vi.fn(), put: vi.fn() },
    settings_cache: { get: vi.fn(), put: vi.fn() },
  },
}));
...
describe('AuthContext', () => {
  ...
  describe('signUp', () => {
    ...
    it('throws when email not on whitelist', async () => {
      ...
      await expect(
        act(async () => {
          await result.current.signUp('nobody@test.com', 'p', 'Nobody');
        })
    ...
    it('throws when whitelist entry already used', async () => {
      mocks.checkWhitelist.mockResolvedValue({
        data: { id: 'reg-1', role: 'manager', used_at: '2024-01-01' },
        error: null,
      });
      ...
      await expect(
        act(async () => {
          await result.current.signUp('used@test.com', 'p', 'X');
        })
    ...
    it('throws when whitelist lookup fails', async () => {
      ...
      await expect(
        act(async () => {
          await result.current.signUp('x@t.com', 'p', 'X');
        })
  ...
  describe('resetPassword', () => {
    it('calls supabase resetPasswordForEmail', async () => {
      ...
      expect(mocks.resetPasswordForEmail).toHaveBeenCalledWith('test@test.com', {
    ...
    it('throws when reset fails', async () => {
      ...
      await expect(
        act(async () => {
          await result.current.resetPassword('bad@test.com');
        })
      ).rejects.toThrow();
    });
  });
  ...
  describe('signOut', () => {
    it('calls supabase signOut + clears sentry', async () => {
      ...
      await act(async () => {
        result.current.updateAuthState({ isLoading: false });
      });
      await act(async () => {
  ...
  describe('updateAuthState', () => {
    it('updates partial state', async () => {
      ...
      await act(async () => {
        result.current.updateAuthState({ userName: 'Updated' });
      });
  ...
  describe('Session Initialization', () => {
    ...
    it('loads user data when session exists', async () => {
      ...
      mocks.getUserProfile.mockResolvedValue({ data: mockManagerProfile, error: null });
      // Make onAuthStateChange invoke callback with SIGNED_IN
