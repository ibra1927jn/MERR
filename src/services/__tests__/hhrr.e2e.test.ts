...
const mockGetExpiredButActive = vi
  .fn()
  .mockResolvedValue([{ id: 'c3', employee_id: 'u2', type: 'permanent', end_date: '2025-01-01' }]);
...
vi.mock('@/repositories/settings.repository', () => ({
  settingsRepository: {
    getByOrchardId: vi.fn().mockResolvedValue({ piece_rate: 7.0 }),
  },
}));

const mockGetBucketCounts = vi
  .fn()
  .mockResolvedValue([
    { picker_id: 'u1' },
    { picker_id: 'u1' },
...
describe('hhrr.service — E2E deep tests', () => {
  ...
  describe('createContract', () => {
    it('adds to sync queue', async () => {
      ...
      expect(vi.mocked(syncService.addToQueue)).toHaveBeenCalledWith(
        'CONTRACT',
        expect.objectContaining({ action: 'create', employee_id: 'u1' })
      );
    });
  });
  ...
  describe('updateContract', () => {
    it('adds update to sync queue', async () => {
      ...
      expect(vi.mocked(syncService.addToQueue)).toHaveBeenCalledWith(
        'CONTRACT',
        expect.objectContaining({ action: 'update', contractId: 'c1' }),
        '2026-01-01'
      );
    });
  });
