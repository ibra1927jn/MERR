...
const mockPicker: Record<string, unknown> = {
  picker_id: 'P01',
  picker_name: 'Alice',
  buckets: 120,
  hours_worked: 40,
...
const mockTeam: Record<string, unknown> = {
  name: 'Carlos Leader',
  buckets: 300,
  bpa: 7.5,
  earnings: 600,
...
describe('getTeamName', () => {
  ...
  it('returns Unassigned when picker has no leader', () => {
    const orphan: Record<string, unknown> = { picker_id: 'P99', picker_name: 'Nobody' };
    expect(getTeamName(orphan, mockCrew)).toBe('Unassigned');
  });

  it('resolves via name fallback', () => {
    const byName: Record<string, unknown> = { picker_id: 'XX', picker_name: 'Bob' };
