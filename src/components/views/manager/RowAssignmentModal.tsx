    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl relative">
                <button onClick={onClose} className="absolute right-4 top-4 text-text-muted">
                    <span className="material-symbols-outlined">close</span>
                </button>

                <h2 className="text-xl font-black text-text-main mb-1">Assign Row {selectedRow}</h2>
                <p className="text-sm text-text-muted mb-6">{orchard?.name || 'Orchard Block'}</p>

                <div className="space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-text-muted uppercase">Select Row</label>
                        <select
                            value={selectedRow}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-text-muted uppercase">Assign Team Leader</label>
                        <select
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-text-muted uppercase">Side</label>
                        <div className="flex bg-slate-100 p-1 rounded-xl">
                            <button
                                onClick={() => setSelectedSide('north')}
                                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${selectedSide === 'north' ? 'bg-white shadow text-primary' : 'text-text-muted'}`}
                            >North Face</button>
                            <button
                                onClick={() => setSelectedSide('south')}
                                className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${selectedSide === 'south' ? 'bg-white shadow text-primary' : 'text-text-muted'}`}
                            >South Face</button>
                        </div>
                    </div>
                </div>
