...
describe('crewSlice', () => {
    ...
    it('provides setPickers action', async () => {
        ...
        if (slice.setPickers) {
            slice.setPickers([{ id: 'p1', name: 'Test', total_buckets_today: 0, hours: 0, status: 'active' }]);
...
describe('settingsSlice', () => {
    ...
    it('creates slice with expected state shape', async () => {
        const mod = await import('./settingsSlice');
        const mockSet = vi.fn();
        const mockGet = vi.fn().mockReturnValue({ settings: {}, orchard: { id: 'o1' }, currentUser: { id: 'u1' } });
        const slice = mod.createSettingsSlice(mockSet, mockGet, {} as never);
        expect(slice).toHaveProperty('settings');
        expect(slice.settings).toHaveProperty('min_wage_rate', 23.50);
    });
