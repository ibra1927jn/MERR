...
SET search_path = public STABLE AS $func$
SELECT orchard_id
FROM public.users
WHERE id = auth.uid()
LIMIT 1;
...
$func$;
-- updated_at trigger
CREATE OR REPLACE FUNCTION public.set_updated_at() RETURNS TRIGGER AS $func$ BEGIN NEW.updated_at = now();
RETURN NEW;
END;
$func$ LANGUAGE plpgsql;
-- Optimistic locking trigger
CREATE OR REPLACE FUNCTION public.bump_version() RETURNS TRIGGER AS $func$ BEGIN IF OLD.version IS NOT NULL
...
SET search_path = public LANGUAGE plpgsql AS $func$
...
SET search_path = public LANGUAGE plpgsql AS $func$ BEGIN RETURN EXISTS (
        SELECT 1
        FROM account_locks
...
CREATE OR REPLACE FUNCTION public.update_day_closures_updated_at() RETURNS TRIGGER AS $func$ BEGIN NEW.updated_at = now();
RETURN NEW;
END;
$func$ LANGUAGE plpgsql;
...
SET search_path = public LANGUAGE plpgsql AS $func$ BEGIN IF NOT EXISTS (
        SELECT 1
        FROM public.users
        WHERE id = auth.uid()
...
SET search_path = public LANGUAGE plpgsql AS $func$ BEGIN
...
END;
$func$;
CREATE OR REPLACE FUNCTION public.cleanup_old_audit_logs() RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $func$ BEGIN
...
SET search_path = public AS $func$ BEGIN RETURN QUERY
SELECT al.id,
    al.action,
    al.user_email,
    al.old_values,
...
-- 14. REALTIME
-- =============================================
DO $func$ BEGIN ALTER PUBLICATION supabase_realtime
