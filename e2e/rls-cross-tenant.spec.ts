...
// ACID Orchards
const _ORCHARD_A = 'a1000000-0000-0000-0000-000000000001'; // ACID_ALPHA
...
test.describe('Cross-Tenant RLS Security', () => {
  ...
  test('CRITICAL: Manager A cannot see Orchard B bucket records (SELECT blocked)', async ({
    page,
  }) => {
    // Login as Manager A
    await page.goto('/login');
    ...
    if (token) {
      ...
      const { data: records, _error } = await supabaseAsManagerA
        .from('bucket_records')
        .select('*')
  ...
  test('CRITICAL: IDOR — Manager A cannot INSERT bucket into Orchard B', async ({ page }) => {
    ...
    if (token) {
      ...
      const { _data, error } = await supabaseAsManagerA.from('bucket_records').insert({
  ...
  test('CRITICAL: IDOR — Manager A cannot INSERT picker into Orchard B', async ({ page }) => {
    ...
    if (token) {
      ...
      const { error } = await supabaseAsManagerA.from('pickers').insert({
  ...
  test('CRITICAL: IDOR — Manager A cannot UPDATE Orchard B attendance', async ({ page }) => {
    ...
    if (token) {
      ...
      const { data, _error } = await supabaseAsManagerA
