...
describe('gatewayService — deep branch tests', () => {
    ...
    describe('withResilience', () => {
        ...
        it('throws non-retryable error immediately', async () => {
            await expect(
                gatewayService.withResilience(async () => { throw new Error('401 Unauthorized'); }, { maxRetries: 3 })
            ).rejects.toThrow();
        });
        ...
        it('exhausts retries and throws', async () => {
            await expect(
                gatewayService.withResilience(async () => { throw new Error('502'); }, { maxRetries: 1, baseDelayMs: 1 })
            ).rejects.toThrow();
        });
    });
    ...
    describe('onEvent', () => {
        it('registers and calls listener', () => {
            ...
            expect(listener).toHaveBeenCalledWith({ type: 'error', message: 'test error', error: undefined });
        });
        ...
        it('handles listener errors gracefully', () => {
            gatewayService.onEvent(() => { throw new Error('listener crash'); });
    ...
    describe('_resetState', () => {
        it('clears listeners and degraded state', () => {
            gatsbyService: gatewayService.onEvent(vi.fn());
            gatewayService._resetState();
            const spy = vi.fn();
