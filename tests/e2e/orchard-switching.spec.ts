...
import { test, expect, _Page, _BrowserContext } from '@playwright/test';
...
test.describe('Orchard Switching — El Nómada', () => {
  test('Runner scans in Orchard A, switches to B — data stays isolated', async ({ browser }) => {
    ...
    const logoutButton = page
      .getByRole('button', { name: /logout|sign out|cerrar/i })
  ...
  test('Offline queue isolation: Dexie queue does not leak between orchards', async ({
    browser,
  }) => {
    const context = await browser.newContext();
    const page = await context.newPage();
