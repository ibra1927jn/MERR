...
vi.mock('@/services/supabase', () => ({
    supabase: {
        ...
        channel: vi.fn().mockReturnValue({
            on: vi.fn().mockReturnThis(), subscribe: vi.fn(),
        }),
...
vi.mock('@/repositories/storeSync.repository', () => ({
    storeSyncRepository: {
        ...
        getSettings: vi.fn().mockResolvedValue({ piece_rate: 3.5, min_wage_rate: 23.50 }),
    ...
    const set = vi.fn((partial: any) => {
        if (typeof partial === 'function') {
            updates.push(partial({
                buckets: [], crew: [], bucketRecords: [],
                rowAssignments: [], lastSyncAt: null,
                recentQcInspections: [], recentTimesheetUpdates: [],
                orchard: null, settings: null,
            }));
        } else {
            updates.push(partial);
        }
...
function makeGet(overrides: Record<string, unknown> = {}) {
    return vi.fn(() => ({
        crew: [], buckets: [], bucketRecords: [],
        rows: [], orchardBlocks: [], orchardMapRows: [],
        orchard: null, settings: null, lastSyncAt: null,
        rowAssignments: [],
        fetchBlocks: vi.fn(),
...
describe('hydrateFromRecovery', () => {
    ...
    it('clears recovery data after consuming', () => {
        mockLocalStorage['harvest-pro-recovery'] = JSON.stringify({ state: { buckets: [{ id: 'b1' }] } });
        const { set } = makeSet();
        hydrateFromRecovery(set);
