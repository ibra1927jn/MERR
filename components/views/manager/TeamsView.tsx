...
const TeamsView: React.FC<TeamsViewProps> = ({ crew, setShowAddUser, setSelectedUser, settings }) => {
    ...
    const { leaders, unassigned, groupedCrew } = useMemo(() => {
        const leaders = crew.filter(p => p.role === Role.TEAM_LEADER || p.role === 'team_leader');
        const runners = crew.filter(p => p.role === Role.RUNNER || p.role === 'runner'); // Kept separate or integrated? Usually separate view/card.
        ...
        crew.forEach(p => {
            // Skip leaders and runners in the "picker list" logic to avoid duplication if role check is fuzzy
        ...
        return { leaders, unassigned: unassignedList, groupedCrew: grouped };
    }, [crew]);

    // 2. Filter Logic (Search only for now, as hierarchy implies structure)
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-black/20">
            ...
            {/* Content: List of Team Leader Cards */}
