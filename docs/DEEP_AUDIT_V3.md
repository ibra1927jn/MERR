...
### BUG-1: Stale Closure en AuthContext (Severidad: Media)
...
if (session?.user && !state.isAuthenticated) {  // ← stale closure
...
### BUG-2: Unsafe User Cast (Severidad: Baja)
...
### BUG-3: Unsafe Casts en Sync Queue Processing (Severidad: Media)
...
### BUG-4: `close_payroll_period` usa piece rate hardcodeado (Severidad: Alta)
...
### BUG-5: `messages` INSERT policy permite spoofing de sender_id (Severidad: Media)
...
### BUG-6: `allowed_registrations` no tiene campo `used_at` en schema
...
### DES-2: Min Wage Discrepancia Documental
...
El salario mínimo NZ 2026 es **$23.15/hr** (actualizado en abril 2025). Todos los archivos usan $23.50 que fue un valor propuesto pero no confirmado.

**Fix:** Centralizar en UN SOLO lugar (`types.ts`) y actualizar al valor real.

---

### DES-3: Visibility Handler sin Cleanup Robusto
...
> **Conclusión:** El código es de calidad producción real. Los 6 bugs encontrados son sutiles (no crashes obvios), lo que indica madurez del codebase. La arquitectura offline-first con DLQ + Web Locks + conflict resolution está al nivel de apps enterprise. Las áreas de mejora son menores: unsafe casts (TypeScript strictness), un valor hardcodeado en SQL, y una policy RLS permisiva.
