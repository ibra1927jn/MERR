...
describe('Sync Retry & DLQ — Integration', () => {
    beforeEach(async () => {
        ...
        try { await db.dead_letter_queue.clear(); } catch { /* may not exist in test env */ }
        vi.clearAllMocks();
    });
    ...
    it('getQueueSummary provides accurate type breakdown', async () => {
        ...
        await syncService.addToQueue('MESSAGE', {
            channel_type: 'direct', recipient_id: 'u2', sender_id: 'u1', content: 'Hi', timestamp: '2026-03-10T10:00:00+13:00',
        });
    ...
    it('getMaxRetryCount returns highest retry count', async () => {
        await db.sync_queue.bulkPut([
            { id: 'r1', type: 'SCAN', payload: SCAN_PAYLOAD as any, retryCount: 2, timestamp: Date.now() },
            { id: 'r2', type: 'SCAN', payload: SCAN_PAYLOAD as any, retryCount: 7, timestamp: Date.now() },
            { id: 'r3', type: 'SCAN', payload: SCAN_PAYLOAD as any, retryCount: 3, timestamp: Date.now() },
        ]);
