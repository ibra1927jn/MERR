...
import { NZ_MINIMUM_WAGE_2025 } from '@/constants/nz-law';
...
export async function fetchHRSummary(orchardId?: string): Promise<HRSummary> {
  try {
    ...
    return {
      ...
      payrollThisWeek: totalHours * NZ_MINIMUM_WAGE_2025,
  ...
  try {
    ...
    return data.map(user => {
      ...
      return {
        ...
        hourly_rate: contract?.hourly_rate || NZ_MINIMUM_WAGE_2025,
...
export async function updateContract(
  ...
  return syncService.addToQueue(
    'CONTRACT',
    { action: 'update', contractId, ...updates },
    currentUpdatedAt
  );
}
...
export async function fetchPayroll(orchardId?: string): Promise<PayrollEntry[]> {
  try {
    ...
    const attendance = await attendanceRepository.getHoursSummary(
      orchardId,
    ...
    attendance.forEach(a => {
      if (a.check_in_time && a.check_out_time) {
        ...
        hoursByPicker[a.picker_id] =
          (hoursByPicker[a.picker_id] || 0) + Math.max(0, Math.min(hrs, 12));
      }
    });

    let pieceRateValue = 6.5;
    if (orchardId) {
...
export async function fetchComplianceAlerts(orchardId?: string): Promise<ComplianceAlert[]> {
  ...
  try {
    ...
    const expiringContracts = await contractRepository2.getExpiringSoon(
      orchardId,
      today,
      thirtyDaysFromNow
    );
