/**
 * PAYROLL.TSX — Payroll Admin Department
 * Dedicated payroll dashboard using DesktopLayout
...
const Payroll: React.FC = () => {
    ...
    useEffect(() => {
        const loadPayroll = async () => {
            ...
            try {
                ...
                console.warn('[Payroll] Failed to load payroll data:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const navItems = PAYROLL_NAV.map(item => ({
        ...item,
        badge: item.id === 'dashboard' && compliance.workers_below_minimum > 0 ? compliance.workers_below_minimum : undefined,
    }));
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
                <SummaryCard icon="inventory_2" iconColor="text-indigo-500" label="Total Buckets" value={summary.total_buckets.toLocaleString()} />
                <SummaryCard icon="schedule" iconColor="text-sky-500" label="Total Hours" value={`${summary.total_hours.toFixed(1)}h`} />
                <SummaryCard icon="payments" iconColor="text-emerald-500" label="Piece Rate" value={`$${summary.total_piece_rate_earnings.toFixed(0)}`} />
                <SummaryCard icon="shield" iconColor="text-amber-500" label="Top-Up Required" value={`$${summary.total_top_up.toFixed(0)}`} />
                <SummaryCard icon="account_balance" iconColor="text-orange-600" label="Total Payroll" value={`$${summary.total_earnings.toFixed(0)}`} highlight />
            </div>

            {/* Compliance Banner */}
            {compliance.workers_below_minimum > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3 mb-4">
                    ...
                    <p className="text-sm font-medium text-amber-800">
                        Wage Shield active for {compliance.workers_below_minimum} of {compliance.workers_total} workers.
                        Compliance rate: {compliance.compliance_rate.toFixed(0)}%
                    </p>
                </div>
            )}
...
/* ── Summary Card ── */
const SummaryCard: React.FC<{ icon: string; iconColor: string; label: string; value: string; highlight?: boolean }> = ({ icon, iconColor, label, value, highlight }) => (
    <div className={`rounded-xl p-4 shadow-sm border ${highlight ? 'bg-orange-50 border-orange-200' : 'bg-white border-gray-100'}`}>
        <div className="flex items-center gap-2 mb-1.5">
            <span className={`material-symbols-outlined text-lg ${iconColor}`}>{icon}</span>
            <span className="text-xs text-gray-500 font-medium">{label}</span>
        </div>
        <p className={`text-2xl font-black ${highlight ? 'text-orange-700' : 'text-gray-900'}`}>{value}</p>
    </div>
);

/* ── Dashboard Tab ── */
const PayrollDashboard: React.FC<{ pickers: PickerBreakdown[]; settings: { bucket_rate: number; min_wage_rate: number } }> = ({ pickers, settings }) => (
    <div className="space-y-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-3">
                <h3 className="font-bold text-gray-900">Picker Breakdown</h3>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-gray-900 font-medium focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 outline-none"
                        />
                    </div>
                    <div>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between py-1.5">
                        <span className="text-sm text-gray-600">Piece Rate ({buckets} × ${settings.bucket_rate})</span>
                        <span className="text-sm font-bold text-gray-900">${pieceRate.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between py-1.5">
                        <span className="text-sm text-gray-600">Minimum Required ({hours}h × ${settings.min_wage_rate})</span>
                        <span className="text-sm font-bold text-gray-900">${minimumRequired.toFixed(2)}</span>
                    </div>
                    {topUp > 0 && (
...
const ExportTab: React.FC = () => (
    <div className="space-y-4 max-w-lg">
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
            <span className="material-symbols-outlined text-gray-300 text-5xl mb-3 block">download</span>
            <h3 className="font-bold text-gray-900 mb-1">Export Payroll Data</h3>
            <p className="text-sm text-gray-500 mb-4">Export payroll data for accounting software integration</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
                ...
                <button
                    key={exp.format}
                    disabled={exp.desc === 'Coming soon'}
                    className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3 hover:shadow-md transition-shadow text-left disabled:opacity-40 disabled:cursor-not-allowed"
                >
                    ...
                    <div>
                        <h4 className="font-bold text-gray-900 text-sm">{exp.label}</h4>
