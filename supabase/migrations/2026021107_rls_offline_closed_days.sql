...
DROP POLICY IF EXISTS "no_insert_on_closed_days" ON bucket_records;
...
WITH CHECK (
  NOT EXISTS (
    ...
    AND day_closures.date = DATE(bucket_records.scanned_at AT TIME ZONE 'Pacific/Auckland')
    AND day_closures.status = 'closed'
    AND bucket_records.scanned_at >= day_closures.closed_at  -- Allow pre-closure offline data
  )
);

-- Comment for audit trail
COMMENT ON POLICY "no_insert_on_closed_days" ON bucket_records IS
