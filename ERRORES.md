...
## P2 — Tests i18n sweep (sesion 2026-04-14 round 3)

- [2026-04-14] | src/test-utils.tsx (ausente) | Tests importando de @testing-library/react directamente no recibían I18nProvider → t(key) devolvía la clave cruda en vez del string traducido → getByText('Sign In') fallaba cuando el componente rendereaba 'auth.sign_in'. PATRÓN: al añadir i18n con t() a un componente, sus tests deben usar el wrapper AllProviders (I18nProvider). Solución: crear src/test-utils.tsx con render() custom que envuelve en I18nProvider; actualizar imports en todos los archivos de test | Fix: src/test-utils.tsx creado; 20+ archivos de test actualizados a `from '@/test-utils'`
- [2026-04-14] | src/components/auth/LoginForm.tsx + tests | isSubmitting=true cambia texto del botón a t('auth.signing_in')='Signing In...' no 'Sign In'. Tests buscaban getByText('Sign In') con isSubmitting=true → fail. PATRÓN: cuando el botón de submit tiene estado de texto dual (idle/loading), los tests de disabled deben usar getByRole('button', { name: /signing/i }) | Fix: tests actualizados a getByRole con regex flexible
- [2026-04-14] | src/services/analytics-trends.service.ts getDailyBleed | Último entry tenía label=abreviatura del día ('Tue') en vez de 'Today'. Test: `expect(result[last].label).toBe('Today')` fallaba. PATRÓN: cuando hay un "today" entry, etiquetarlo explícitamente | Fix: `const label = i === 0 ? 'Today' : weekdayFmt.format(d)`
- [2026-04-14] | src/services/picker-history.service.ts | `hours = (Date.now() - check_in_time) / 3600000` cuando check_out_time=null → para fechas pasadas devuelve días/semanas de horas (843h para 2026-03-10). Test esperaba 0h. PATRÓN: el cálculo live-hours solo aplica hoy, no para fechas históricas sin check-out | Fix: `const isToday = a.date === todayStr; ... a.check_in_time && isToday ? Math.max(0, ...) : 0`
- [2026-04-14] | src/components/common/PickerProfileDrawer.test.tsx | vi.mock('./picker-profile') solo incluía Sparkline/QualityRing/RiskBadge/TabButton — faltaban PickerPanel, HistoryTab, QualityTab, RunnerPanel, TeamLeaderPanel. PickerProfileDrawer los usaba → Error: No "PickerPanel" export defined on mock | Fix: añadir todos los exports del index.ts al mock
- [2026-04-14] | src/pages/Manager.integration.test.tsx | vi.mock('@/components/common/PickerProfileDrawer') pero Manager.tsx importa de '@/components/PickerDrawer' (export named PickerDrawer). El mock apuntaba al módulo equivocado → data-testid="profile-drawer" nunca aparecía en el DOM | Fix: añadir vi.mock('@/components/PickerDrawer', () => ({ PickerDrawer: () => createElement('div', { 'data-testid': 'profile-drawer' }) }))

## Bugs sesion 2026-04-16

- [2026-04-16] | src/hooks/useWeeklyReport.ts:278 | openProfile buscaba en crew por `c.picker_id` (ID legible "402") pero `p.picker_id` de bucket_records es `pickers.id` (UUID = `c.id`). La búsqueda siempre retornaba undefined → `if(picker)` nunca verdadero → panel lateral nunca abría desde Weekly Report. PATRÓN: `bucket_records.picker_id` es el UUID del registro pickers, NO el ID legible del sticker | Fix: `openPickerProfile(pickerId)` directo sin búsqueda — el UUID ya es el id correcto
- [2026-04-16] | src/hooks/useSettings.ts:180 | handleOrchardSelect actualizaba `useHarvestStore.setState({orchard})` pero no llamaba `fetchBlocks(id)` → al cambiar de huerto el mapa seguía mostrando los bloques del huerto anterior. PATRÓN: cualquier cambio de orchard activo debe redispachar fetchBlocks para recargar bloques/filas | Fix: `void useHarvestStore.getState().fetchBlocks(selected.id)` tras setState
- [2026-04-16] | LogisticsView + src/hooks/useLogistics.ts | Código correcto pero tablas `bins`, `fleet_vehicles`, `transport_requests` vacías en seed local → todas las métricas en 0 / "Sin datos". No es bug de código. PENDIENTE: agregar seed data de logística cuando corresponda

## P0 — Bugs sesion 2026-04-16 (DB)

- [2026-04-16] | supabase/migrations/ | `orchards` table nunca tuvo columnas `crop_type` ni `deleted_at`. `useSettings.ts:124` hacía `.select('id,name,total_rows,crop_type').is('deleted_at',null)` → PostgREST 400 en cada carga. La app arrancaba pero no podía leer orchard data. PATRÓN: al escribir queries que incluyan columnas opcionales o soft-delete, verificar que existen en el schema base antes del primer deploy | Fix: migración `20260416000001` añade `crop_type TEXT` y `deleted_at TIMESTAMPTZ` a orchards + índice parcial. Tipos regenerados.

## P0 — Bugs sesion 2026-04-14 v4

- [2026-04-14] | src/utils/time.ts:toNZISO | getNZOffset() calculaba offset NZ con `new Date(y,m,d,h,m,s).getTime()` (local timezone). En máquinas con timezone=Pacific/Auckland, "NZ local time" == "UTC local time" → diff=0 → offset="+00:00". En CI (UTC) daba +12 correcto. En máquina del developer (NZ) daba +0 roto. PATRÓN CRÍTICO: cuando se calcula un offset de timezone, NUNCA usar el constructor `new Date(y,m,d,...)` (depende de timezone local). Siempre usar `Date.UTC(y,m,d,...)` para interpretar partes de fecha como UTC independientemente de la máquina | FIXED [2026-04-14]: `const nzAsUtcMs = Date.UTC(...)` en lugar de `new Date(...).getTime()`. Todos los tests de projection.ts y hours.ts pasan ahora en NZ timezone y UTC

- [2026-04-14] | src/components/views/manager/DashboardView.test.tsx | Test "shows active crew count" esperaba 12 (de presentCount prop) pero activeCrew ahora se deriva de bucketRecords únicos. Con los 3 records mock (todos picker_id='p-1'), activeCrew=1. PATRÓN: al cambiar la fuente de datos de un stat card en producción, actualizar también la expectation en el test | FIXED [2026-04-14]: Expectation actualizada a 1 (1 picker único en bucketRecords)
- [2026-04-17] | src/types/database.types.ts | Al regenerar con supabase gen types se borraron las funciones helper manuales (isSupabasePicker, isSupabaseUser, isSupabaseChatMessage, isSupabaseAttendanceRecord + arrays) → CI falla con "is not a function" | Añadir comentario "NO borrar al regenerar" y restaurar las funciones al final del archivo
- [2026-04-17] | src/types/database.types.ts | Segunda omisión en regeneración: type aliases SupabasePicker y SupabasePerformanceStat también se borran al regenerar → TS2724/TS2305 en attendance.service y picker.service | Añadir type aliases al bloque manual junto a los type guards
